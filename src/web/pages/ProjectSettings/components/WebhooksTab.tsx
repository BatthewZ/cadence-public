...
import { CreateWebhookDialog } from "../../WorkspaceSettings/components/CreateWebhookDialog";
import { DeleteWebhookDialog } from "../../WorkspaceSettings/components/DeleteWebhookDialog";
import { EditWebhookDialog } from "../../WorkspaceSettings/components/EditWebhookDialog";
import {
  type TestDeliveryResult,
  TestResultDisplay,
} from "../../WorkspaceSettings/components/TestResultDisplay";
import {
  deliveryColumns,
  type DeliveryRow,
} from "../../WorkspaceSettings/components/WebhookColumns";
...
export function WebhooksTab({
  ...
  const {
    ...
    queryFn: () =>
      api.get<{ webhooks: WebhookRow[] }>(`/api/projects/${projectId}/webhooks`),
    enabled: !!projectId,
    staleTime: 30_000,
  });
  ...
  const { data: detailData, isLoading: detailLoading } = useQuery({
    queryKey: queryKeys.projects.webhookDetail(projectId, selectedWebhookId ?? ""),
    queryFn: () =>
      api.get<{ webhook: WebhookRow; deliveries: DeliveryRow[] }>(
        `/api/projects/${projectId}/webhooks/${selectedWebhookId}`,
      ),
    enabled: !!projectId && !!selectedWebhookId,
    staleTime: 15_000,
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateWebhookInput) =>
      api.post<{ webhook: WebhookRow }>(`/api/projects/${projectId}/webhooks`, data),
    onSuccess: (result) => {
      void qc.invalidateQueries({ queryKey: queryKeys.projects.webhooks(projectId) });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      api.delete(`/api/projects/${projectId}/webhooks/${id}`),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: queryKeys.projects.webhooks(projectId) });
            ...
            <Card>
              <Stack gap="r4">
                ...
                <Row gap="r5" className="flex-wrap">
                  <Button variant="secondary" size="sm" onClick={() => handleOpenEdit(wh)}>
                    Edit
                  </Button>
                  <Button
                    variant="secondary"
                  ...
                  <Button variant="danger" size="sm" onClick={() => setDeleteTarget(wh)}>
    ...
    <>
      ...
      {listLoading ? (
        ...
        <EmptyState size="md">
          ...
          <EmptyStateDescription>
            Create a webhook to start receiving event notifications for this project.
          </EmptyStateDescription>
        </EmptyState>
      ) : (
        <Stack gap="r5">
              ...
              >
                <Row justify="between" align="center" className="flex-wrap gap-r5">
                  ...
                  <Row gap="r5" align="center" className="shrink-0">
                    ...
                    {wh.consecutiveFailures > 0 && (
                      <Badge variant="warning">{wh.consecutiveFailures} failures</Badge>
                    )}
                  </Row>
                </Row>
              </button>
